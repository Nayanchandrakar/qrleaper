...
- A Next.js middleware handler resolves the current destination at scan time and redirects.
...
```mermaid
flowchart LR
    Q["Printed QR code encodes /link?id=..."] --> M["middleware.ts"]
...
1. **`/link?id=<id>`** (and **`/vcard/<username>`**, which first resolves the slug to a QR code id) is intercepted by `middleware.ts`.
2. The middleware loads the QR code and the owner's subscription. Inactive codes, lapsed subscriptions, and free-tier codes past 500 scans are redirected to `/expired` instead of the destination.
...
| --- | --- | --- |
| Framework | Next.js 14 (App Router), React 18 | Server components for data fetching, server actions for mutations |
| Language | TypeScript 5.9 | Strict mode; shared types across client and server |
| Database | PostgreSQL (Neon serverless driver) | Drizzle ORM + drizzle-kit migrations |
| Auth | NextAuth.js v5 (beta) | Google, Resend magic link, and credentials providers; JWT sessions; Drizzle adapter. Config is split so the edge middleware doesn't import the database adapter |
...
| Variable | Purpose |
| --- | --- |
| `DATABASE_URL` | PostgreSQL connection string used by Drizzle and the Neon serverless pool |
| `AUTH_SECRET` | NextAuth.js secret used to sign sessions and tokens |
| `AUTH_TRUST_HOST` | Set to `true` when self-hosting behind a proxy so NextAuth trusts forwarded headers |
| `APP_URL`, `NEXT_PUBLIC_APP_URL` | Public base URL of the deployment; used to build QR endpoints, edit links, and email links |
| `CRON_SECRET` | Bearer token required by the cron API route |
| `AUTH_GOOGLE_ID`, `AUTH_GOOGLE_SECRET` | Google OAuth credentials |
| `RESEND_API_KEY`, `RESEND_MAIL` | Resend API key and the `From` address for outgoing email |
| `AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY`, `AWS_REGION`, `S3_BUCKET` | S3 credentials and bucket for uploads and generated assets |
| `STRIPE_API_KEY`, `STRIPE_WEBHOOK_SECRET` | Stripe server key and webhook signing secret |
| `NEXT_PUBLIC_STRIPE_{STARTER,PLUS,PRO}_{MONTHLY,YEARLY}_PLAN_ID` | Six Stripe price IDs mapped to plans in `constants/pages/pricing/` |
...
The Free tier additionally caps codes at 500 scans (enforced in the redirect middleware) and its pricing copy describes a 30-day trial with 30 days of analytics.
...
3. **Scan** - a scan hits the middleware, which records analytics and redirects to the current destination. vCard codes also expose a public profile at `/vcard/<username>` with an "add to contacts" `.vcf` download.
...
| `/link?id=<id>` | Dynamic redirect (handled entirely by middleware - no page component) |
| `/vcard/<username>` | Public vCard slug (middleware resolves it to the link pipeline) |
...
routes.ts        Route lists and regexes consumed by middleware
middleware.ts    Auth-aware request routing and redirect pipeline entry point
```
...
- **Type checking is not part of the build.** `next.config.mjs` sets `typescript.ignoreBuildErrors: true`, so type errors do not fail `next build`.
- **Subscription gating on `/edit` routes is primarily enforced by server actions.** The middleware pattern for edit routes matches paths beginning with `/edit`, while edit pages are actually served from `/[id]/edit`; action-level guards (`throwSubscriptionEditError`) and the create-time quota check are the effective enforcement points.
